...
public class DatabasePanelFrom extends JPanel {
    ...
    private static JComboBox<String> comboxDatabaseType;
    ...
    private JPanel getCenterPanel() {
        ...
        comboxDatabaseType = new JComboBox<String>();
        comboxDatabaseType.addItem("Oracle");
    ...
    public static void setContent() {
        String typeFrom = ConstantsTools.CONFIGER.getTypeFrom();
        comboxDatabaseType.setSelectedIndex(typeFrom.trim().toUpperCase().equals("MySQL".toUpperCase()) ? 1 : (typeFrom.trim().toUpperCase().equals("SQL Server".toUpperCase()) ? 2 : 0));
    ...
    private void addListener() {
        buttonSave.addActionListener(e -> {
            ...
            try {
                ConstantsTools.CONFIGER.setTypeFrom(comboxDatabaseType.getSelectedItem().toString());
        ...
        buttonTestLink.addActionListener(e -> {
            ...
            try {
                // DbUtilOracle dbOracle = DbUtilOracle.getInstance();
                DbUtil dbFrom = null;
                switch(String.valueOf(comboxDatabaseType.getSelectedItem()).replace(" ", "").toUpperCase()){
                    case "MYSQL" :
                        dbFrom = DbUtilMySQL.getInstance();
                        break;
                    case "SQLSERVER" :
                        dbFrom = DbUtilSQLServer.getInstance();
                        break;
                    default :
                        dbFrom = DbUtilOracle.getInstance();
                }
                ...
                Connection conn = dbFrom.testConnection(dburl, dbname, dbuser, dbpassword);
                if (conn == null) {
