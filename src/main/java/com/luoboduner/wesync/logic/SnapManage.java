...
public class SnapManage {
     ...
     * 创建快照
     *
     * @param originalTablesMap：来源表Map
     * @return
     */
    public static boolean createSnap(Map<String, Table> originalTablesMap, Map<String, Table> targetTablesMap) {
        ...
        StatusPanel.progressCurrent.setMaximum(originalTablesMap.keySet().size());
        int progressValue = 0;
        StatusPanel.progressCurrent.setValue(progressValue);

        // 遍历来源表Map，建立快照
        for (String snapName : originalTablesMap.keySet()) {
            if (!"".equals(snapName.trim())) {
                ...
                DbUtilOracle oracle = null;
                DbUtilMySQL mysql = null;
                try {
                    String primKeys = ExecuteThread.originalTablesMap.get(snapName).getPrimKey();
                    StatusLog.setStatusDetail(primKeys, LogLevel.ERROR);
                    String timestamp = "";
                    Long samecount = new Long(0);
                    ArrayList<String[]> snapBeforeList = new ArrayList<String[]>();
                    if (snapBefore.exists()) {
                        snapBeforeList = FileUtils.getCsvFileContentList(snapBefore);
                    }
                    if (snapBefore.exists() && snapBeforeList.size() > 1) {
                        String[] r = snapBeforeList.stream().skip(1).max(Comparator.comparing((String[] row) -> new java.math.BigInteger(row[2].replace("-", "").replace(":", "").replace(".", "").replace(" ", "")))).get();
                        timestamp = r[2];
                        final String final_timestamp = timestamp;
                        samecount = snapBeforeList.stream().skip(1).filter((String[] row) -> row[2].equals(final_timestamp)).count();
                    }
                    else {
                        mysql = DbUtilMySQL.getInstance();
                        String querySql = "SELECT MAX(UPDATETIME) as LAST_TIMESTAMP FROM "
                                + targetTablesMap.get(snapName).getTableName();
                        ResultSet rs = mysql.executeQuery(querySql);
                        rs.next();
                        timestamp = rs.getString("LAST_TIMESTAMP");
                        timestamp = timestamp == null ? "1900-01-01 00:00:00" : timestamp;

//                        querySql = "SELECT COUNT(1) as COUNT_THE_SAME FROM "
//                                + targetTablesMap.get(snapName).getTableName()
//                                + " WHERE UPDATETIME='" + timestamp + "'";
//                        rs = mysql.executeQuery(querySql);
//                        rs.next();
//                        samecount = rs.getInt("COUNT_THE_SAME");
//                        StatusLog.setStatusDetail("COUNT_THE_SAME:" + String.valueOf(samecount), LogLevel.ERROR);
                    }
                    samecount = timestamp.equals(ConstantsTools.CONFIGER.getLastTimeStamp()) ? samecount + Integer.valueOf(ConstantsTools.CONFIGER.getBreakPoint()) : samecount;
                    StatusLog.setStatusDetail(timestamp, LogLevel.ERROR);
                    StatusLog.setStatusDetail("COUNT_THE_SAME:" + String.valueOf(samecount), LogLevel.ERROR);
                    try{
                        ConstantsTools.CONFIGER.setLastTimeStamp(timestamp);
                        ConstantsTools.CONFIGER.setBreakPoint(String.valueOf(samecount));
                    } catch (Exception e) {
                    ...
                    // 获取Oracle连接实例
                    oracle = DbUtilOracle.getInstance();

                    String querySql = "SELECT " + originalTablesMap.get(snapName).getFields() + " FROM (SELECT " + originalTablesMap.get(snapName).getFields() +
                            ",ROW_NUMBER() OVER(PARTITION BY UPDATETIME ORDER BY " + primKeys + ") RN"
                            + " FROM "
                            + originalTablesMap.get(snapName).getTableName();
                    if (originalTablesMap.get(snapName).getOther() != null) {
                        querySql += (" " + originalTablesMap.get(snapName).getOther());
                        if (!timestamp.equals("")) {
                            querySql = querySql.replaceAll("(?i) WHERE ", " WHERE UPDATETIME>=timestamp'" + timestamp + "' AND ");
                        }
                    }
                    else if (!timestamp.equals("")) {
                        querySql += (" AND UPDATETIME>=timestamp'" + timestamp + "'");
                    }
                    querySql += ") T WHERE RN>CASE WHEN UPDATETIME=timestamp'" + timestamp + "' THEN " + samecount + " ELSE 0 END "
                            + "AND ROWNUM <= 1000"
                            + "ORDER BY UPDATETIME,STK_UNI_CODE,END_DATE";
                    StatusLog.setStatusDetail(querySql, LogLevel.ERROR);
                    // 表查询
                    ResultSet rs = oracle.executeQuery(querySql);
                    // 获取列信息
                    ResultSetMetaData m;
                    m = rs.getMetaData();
     ...
     * @return
     */
    public static boolean diffSnap(Map<String, Table> originalTablesMap) {
            ...
            if (!"".equals(snapName.trim())) {
                ...
                if (snapMD5Before.equals(snapMD5Now)) {
                    ...
                    try {
                        ...
                        if (TriggerManage.Oracle != null) {
                            try {
                                TriggerManage.Oracle.close();
                            } catch (SQLException e) {
                                StatusLog.setStatusDetail(PropertyUtil.getProperty("ds.logic.lineByLineDiffFail") + e.toString(), LogLevel.ERROR);
                                e.printStackTrace();
                            }
