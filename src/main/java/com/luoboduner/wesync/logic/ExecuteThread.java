...
//        DbUtilSQLServer dbSQLServer = null;
        ...
        DbUtil dbTo = null;
        try {
            // dbOracle = DbUtilOracle.getInstance();
            switch(ConstantsTools.CONFIGER.getTypeFrom().replace(" ", "").toUpperCase()){
                case "MYSQL" :
            ...
            Connection connFrom = dbFrom.testConnection();
            StatusPanel.progressCurrent.setValue(1);
            // dbMySQL = DbUtilMySQL.getInstance();
            switch(ConstantsTools.CONFIGER.getTypeTo().replace(" ", "").toUpperCase()){
                case "MYSQL" :
                    dbTo = DbUtilMySQL.getInstance();
                    break;
                case "SQLSERVER" :
                    dbTo = DbUtilSQLServer.getInstance();
                    break;
                default :
                    dbTo = DbUtilOracle.getInstance();
            }
            Connection connTo = dbTo.testConnection();
            StatusPanel.progressCurrent.setValue(2);
            if (connFrom == null) {
                isLinkedPass = false;
                StatusLog.setStatusDetail("connFrom " + PropertyUtil.getProperty("ds.logic.testLinkFail"), LogLevel.ERROR);
            }

            if (connTo == null) {
                isLinkedPass = false;
                StatusLog.setStatusDetail("connTo " + PropertyUtil.getProperty("ds.logic.testLinkFail"), LogLevel.ERROR);
            }
    ...
    public boolean newSnap() {
        ...
        isSuccess = SnapManage.createSnap(originalTablesMap, targetTablesMap);
        if (isSuccess) {
